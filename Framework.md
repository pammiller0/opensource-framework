...
### Licensing 
In general, works of the United States Government are not entitled to copyright protections in the United States. See 17 U.S.C. § 105. When your program is considering opening its code base to the public, a balance must be achieved between copyright ownership and the Federal Acquisition Regulation (FAR), specifically Section 14 (data rights) and Section 17 (special works). Section 17 allows for the distribution of software but the contractor would retain the copyright. The program should work with a Contracting Officer with knowledge of the special works Sections of the FAR so that the data rights are appropriate to be open sourced. 

As a best practice, such works should be licensed with a CC0 license. The current version of the CC0 license is 1.0. Refer to [creativecommons.org] (https://creativecommons.org/) for the most recent version of the license. To accommodate code contributions, consider adding a `CONTRIBUTION.md` stating that contributors agree to license their work in the same manner, in the root of the repository in addition to the `LICENSE.md` (best practice is not not edit the license file and to add an addtional file with specifics on contributing to the project). In addition, GitHub will add a link to that file at the top of the forms for issues and pull requests, so potential contributors will be informed that the repository has a contribution policy. Under this approach, it's a good idea to reference both documents in `README.md`.
